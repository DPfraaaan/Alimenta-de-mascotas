...
void lcd_i2c_cursor_set(lcd_i2c_handle_t * lcd, uint8_t column, uint8_t row) {
  if (lcd->size == DISPLAY_16X02) {
    if (row == 0) 
      lcd_i2c_write(lcd, 0, 0x80 + column); // Primera fila (0x80 es la base para la fila 1)
    else 
      lcd_i2c_write(lcd, 0, 0x80 + 0x40 + column); // Segunda fila (0x40 es la base para la fila 2)
  }
  else if (lcd->size == DISPLAY_20X04) {
    // Esto se queda igual para el caso del display 20x4
    switch (row) {
      case 0:
...
// FIM DA BIBLIOTECA DO LCD ----------------------------------------------FOUND IN WOKWI
